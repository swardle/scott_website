...
function ReflectVector(d, n) {
    let dn = -2 * Dot(d, n);
    let vnew = [dn * n[0] + d[0], dn * n[1] + d[1]];
    return vnew;
}
function LineVsBoxInside(a, b) {
    let rdir = NormalizeLine(a);
    let dfx = 90000.0;
    let dfy = 90000.0;
    ...
    let t1 = (b.X - a.X2) * dfx;
    let t2 = (b.X + b.Width - a.X2) * dfx;
    let t3 = (b.Y + b.Height - a.Y2) * dfy;
    let t4 = (b.Y - a.Y2) * dfy;
    let xdir = [1, 0];
    let ydir = [0, 1];
    let tdir = [0, 1];
...
class Game {
    // Game::Game
    constructor(canvas, backcanvas, frontctx, backctx) {
...
function newGame() {
    ...
    if (canvas.getContext) {
        ...
        let frontctx = canvas.getContext('2d');
        if (gGame === null) {
            console.log("myNewAnim");
