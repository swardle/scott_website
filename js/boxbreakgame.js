...
Game.prototype.Draw = function()
{
	...
	for (var by = 0; by < this.Field.length; by++) {
		row = this.Field[by];
	...
	for (by = 0; by < this.Field.length; by++) {
		...
		for (bx = 0; bx < row.length; bx++) {
            if(row[bx] == '*')
			{
				...
				var a = new Line(this.Ball.X-this.BallVelocityX, 
					this.Ball.Y-this.BallVelocityY, this.Ball.X, this.Ball.Y);
				var HitLoc = LineVsBox(a,testBox);
				if(HitLoc.IsHit)
				{
					row[bx] = '_';
					var ballv = [this.BallVelocityX, this.BallVelocityY];
					var vnew = ReflectVector(ballv, HitLoc.Normal);
	...
	var a = new Line(this.Ball.X-this.BallVelocityX, 
		this.Ball.Y-this.BallVelocityY, this.Ball.X, this.Ball.Y);
	var HitLoc = LineVsBox(a,this.Player);
	if(HitLoc.IsHit)
	{
		var ballv = [this.BallVelocityX, this.BallVelocityY];
		var vnew = ReflectVector(ballv, HitLoc.Normal);
